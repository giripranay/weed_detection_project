...
def interpolate_pose(start_pose, end_pose, num_steps):
    ...
    for step in range(1, num_steps + 1):
        # Interpolate x, y, and yaw while keeping z constant
        current_pose = [
            start_val + step * step_size[index] if index != 2 else start_val for index, start_val in enumerate(start_pose)
        ]
...
#    intermediate_poses = interpolate_pose([-3.7375 ,-1.47633, 1.13156, 1.56832], [target_x, target_y, target_z, target_yaw], num_steps)
    start_pose = [-3.80000, 8.746210, 1.13156, 1.6]
